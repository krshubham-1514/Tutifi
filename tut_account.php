...
if(isset($_POST['submit']))
{
  ...
  $active=0;
  $query="SELECT * FROM tuts_profile WHERE tut_mob='$mobile'";
  ...
  elseif(mysqli_num_rows($result)>0 && $row['active']==0)
  {
    ...
    {
      echo "my name is shubham kumar sinha";
      $query_1="UPDATE tuts_profile
      SET
      tut_name='$name',
      tut_pass='$password',
      token_email='$token',
    ...
    if(mysqli_query($conn,$query_1))//starting the query
    {
       ...
       //send sms to enter number of the user
        ...
        $query_2="UPDATE tuts_profile SET token_otp='$otp' WHERE tut_mob=$mobile";
    ...
    if($result==true)
    {
      $_SESSION['tut_acc']=$mobile;
      header("Location:verify_mob.php");
    }
    else
    {
  ...
  if($_POST['password1']==$_POST['password2'])//checking that the two entered password matches.
  {
  $query="INSERT INTO tuts_profile (tut_name,tut_pass,tut_mob,token_email,active,email) VALUES ('$name','$password','$mobile','$token','$active','$email')";
...
<body>
  <div class="alert alert-danger" id="message">
	...
	<ul>
	<li><h4>Create tutor Account</h4></li>
  ...
  <li><input type="email" name="email" value="" placeholder=" Email" id="email" required ><br></li>
	...
	<li><input type="submit" name="submit" value="Continue" onclick="return check();" style="background-color:#cc324b;border:0;color:white;" id="btn"><br></li>
	 <li><p>Already have a account?</p><a href="tutor_login.php">Login</a></li>
	</ul>
	</form>
</body>
